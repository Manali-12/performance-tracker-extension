(() => {
  ...
  function finalizeSession(reason: string): void {
    ...
    if (reason === 'beforeunload' && navigationType === 'hard') {
      const isComplete = document.readyState === 'complete';
      const total = loadMetrics?.totalLoadTime ?? 0;
      const hasAnyVitals = fcpValue > 0 && lcpValue > 0 && clsValue > 0 && tbtValue > 0;
      if (!isComplete || !Number.isFinite(total) || total <= 0) {
        return;
      }
